...
public class Database_Listeners extends BaseSubPage {
	...
	public void loadContent() {
		final Label label1 = new Label("Database Listeners can be used to run code automatically for various events. "
				+ "There are currently only Events added at Database level. The Events that have hooks are:");
		addComponent(label1);
		for (final Events event : Database.Events.values()) {
			final Label label2 = new Label(event.name());
			...
			case BEFORE_DELETE_DOCUMENT:
				eventExplanation = "Triggered at the " + beforeAfter + " of the " + FactoryUtils.addCodeString("Document.remove()") + " or "
						+ FactoryUtils.addCodeString("Document.removePermanently()") + " methods. The source will be the document " + "and the target will be the database.";
				break;
			...
			final Label label3 = new Label(eventExplanation + "<br/>", ContentMode.HTML);
			addComponents(label2, label3);
		}
		final Label label4 = new Label("A Listener must implement the IDominoListener interface. The IDominoListener interface has two methods:<ul>" + "<li>"
				+ FactoryUtils.addCodeString("getEventTypes()") + " - returns a List of EnumEvents managed by this Listener. " + "(EvenEvent is the interface which"
				+ FactoryUtils.addCodeString("org.openntf.domino.ext.Database.Events") + " implements.)</li>" + "<li>" + FactoryUtils.addCodeString("eventHappened(IDominoEvent)")
				+ " - runs the code, passing the current EnumEvent being processed." + " Use an 'if' statement to check which event is being triggered and run the relevant code.");
