...
public class Xots_Summary extends BaseSubPage {
	...
	public void loadContent() {
		final Label label1 = new Label("Xots is designed as an all-encompassing replacement for multi-threaded processing, asynchronous jobs, "
		...
		final Label label2 = new Label("The Xots service should be started automatically by the platform's implementation of ODA and you can access it via "
				+ FactoryUtils.addCodeString("Xots.getService()") + ". From there you can call " + FactoryUtils.addCodeString("submit(Runnable)") + " or "
				+ FactoryUtils.addCodeString("submit(Callable)") + " to register and trigger a Xots tasklet to the service. "
				+ "See the sub-pages for examples and information about creating the tasklet classes.");
		label2.setContentMode(ContentMode.HTML);
		final Label label3 = new Label("Architecture");
		label3.setStyleName(ValoTheme.LABEL_H3);
		final Label label4 = new Label("Xots Tasklets are Java classes implementing Callable (to wait for a return value) or Runnable (for background tasks). "
		...
		final Label label5 = new Label("Limitations (as at ODA 2.0.0)");
		label5.setStyleName(ValoTheme.LABEL_H3);
		final Label label6 = new Label("<ul><li>Scheduler is not yet implemented</li><li>TRUSTED session type is not yet implemented</li>" + "<li>The "
		...
		final Label label7 = new Label("Xots Tasklet Retrieving Current Username");
		label7.setStyleName(ValoTheme.LABEL_H3);
		final Label label8 = testSessionPassing();
	...
	private Label testSessionPassing() {
		final Future<String> future = Xots.getService().submit(new SessionCallable());
		final Label label6 = new Label("XOTS FAILED");
		try {
