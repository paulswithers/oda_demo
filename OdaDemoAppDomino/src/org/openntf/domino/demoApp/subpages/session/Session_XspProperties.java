...
public class Session_XspProperties extends BaseSubPage {
	...
	public void loadContent() {
		...
		Label label1 = new Label("XPages applications should enable fixes by setting Xsp Properties accordingly."
				...
				+ FactoryUtils.addCodeString("org.openntf.domino.xsp=godmode,marcel,khan,bubbleExceptions")
				+ "<br/>The switches available are:", ContentMode.HTML);

		Label label2 = new Label("GODMODE");
		label2.addStyleName(ValoTheme.LABEL_H3);
		Label label3 = new Label(
				"Overrides the XPages <i>session</i> and <i>database</i> objects, "
		...
		Label label5 = new Label("Turns on all \"improvements\" like forcing "
				+ FactoryUtils.addCodeString("setAutoUpdate(false)") + " for all view actions.");
		label5.setContentMode(ContentMode.HTML);

		Label label6 = new Label("MARCEL");
		label6.addStyleName(ValoTheme.LABEL_H3);
		Label label7 = new Label("Auto-handles converting session to handle MIME where appropriate."
				+ "The core API does not enforce setting " + FactoryUtils.addCodeString("Session.setConvertMIME()")
		...
		Label label13 = new Label(
				"Does not automatic conversion of Items to MIME. By default, if 32k limit on a field is hit or "
						+ "any other exception is hit, the Item will be converted to MIME.");
		label13.setContentMode(ContentMode.HTML);

		Label label14 = new Label("AUTOMIME32K");
		...
		addComponents(label1, label2, label3, label4, label5, label6, label7, label8, label9, label10, label11, label12,
				label13, label14, label15, new Html_Separator(SeparatorType.NEW_LINE));
	}
