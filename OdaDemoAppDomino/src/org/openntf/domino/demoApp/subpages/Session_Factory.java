...
public class Session_Factory extends BaseSubPage {
	...
	public void loadContent() {
		...
		final Label label1 = new Label("HISTORICAL XPAGES ACCESS");
		label1.addStyleName(ValoTheme.LABEL_H3);
		final Label label2 = new Label(
				"In previous releases there were a number of ways to access sessions:" + "<ul><li>In XPages resolving the variables " + FactoryUtils.addCodeString("session")
						+ " / " + FactoryUtils.addCodeString("opensession") + "</li>" + "<li>" + FactoryUtils.addCodeString("Factory.getSession()") + " and variants</li></ul>");
		...
		if (!Target.NON_XPAGES.equals(currTarget)) {
			final Label label3 = new Label("XPAGES ACCESS");
			label3.addStyleName(ValoTheme.LABEL_H3);
			final Label label4 = new Label("In XPages, resolving the " + FactoryUtils.addCodeString("session / opensession") + "variables is still a valid approach.<br/>"
					+ FactoryUtils.addCodeString("Factory.getSession()") + " will still work, but is now deprecated." + "Since RC3 (release candidate for M1.5) the "
		...
		if (!Target.XPAGES.equals(currTarget)) {
			final Label label5 = new Label("NON-XPAGES ACCESS");
			...
			sb.append("<ul><li><b>CrossWorlds</b> uses a Filter to filter all HTTP requests for any web application, see " + FactoryUtils.addCodeString("org.openntf.xworlds.core")
					+ " bundle's " + FactoryUtils.addCodeString("org.openntf.xworlds.appservers.webapp.XWorldsRequestFilter")
			...
			sb.append("<li><b>The graph REST API</b> uses an extension to " + FactoryUtils.addCodeString("com.ibm.domino.das.servlet.DasServlet") + ", see"
					+ FactoryUtils.addCodeString("org.openntf.domino.rest.servlet.ODADataServlet") + " class. This class's " + FactoryUtils.addCodeString("doService")
					+ " method creates a Session as the current user using the " + FactoryUtils.addCodeString("org.openntf.domino.xsp.session.DasCurrentSessionFactory")
		...
		final Label label6 = new Label("NEW APPROACH");
		...
		for (final SessionType sessType : SessionType.values()) {
		...
		sb.append("</ul>");
		final Label label7 = new Label(sb.toString());
