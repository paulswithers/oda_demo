...
public class Session_XspProperties extends BaseSubPage {
	...
	public void loadContent() {
		...
		final Label label1 = new Label(
				"XPages applications should enable fixes by setting Xsp Properties accordingly." + "The switches should be comma-delimited in the format:<br/>"
		...
		final Label label2 = new Label("GODMODE");
		label2.addStyleName(ValoTheme.LABEL_H3);
		final Label label3 = new Label("Overrides the XPages <i>session</i> and <i>database</i> objects, "
				+ "converting the lotus.domino objects to their org.openntf.domino counterparts. " + "If the switch is not enabled, the org.openntf.domino counterparts are loaded "
				+ "in addition to the lotus.domino objects, as <i>opensession</i> and <i>opendatabase</i>");
		label3.setContentMode(ContentMode.HTML);

		final Label label4 = new Label("KHAN");
		label4.addStyleName(ValoTheme.LABEL_H3);
		final Label label5 = new Label("Turns on all \"improvements\" like forcing " + FactoryUtils.addCodeString("setAutoUpdate(false)") + " for all view actions.");
		label5.setContentMode(ContentMode.HTML);

		final Label label6 = new Label("MARCEL");
		label6.addStyleName(ValoTheme.LABEL_H3);
		final Label label7 = new Label("Auto-handles converting session to handle MIME where appropriate." + "The core API does not enforce setting "
				+ FactoryUtils.addCodeString(">Session.setConvertMIME()") + "when handling MIME items and unsetting it once complete, but failing to do so can cause problems, "
				+ "e.g. accessing two different MIME items at once can cause a JRE and server crash.");
		label7.setContentMode(ContentMode.HTML);

		final Label label8 = new Label("RAID");
		label8.addStyleName(ValoTheme.LABEL_H3);
		final Label label9 = new Label("Turns on debug level messages");

		final Label label10 = new Label("BUBBLEEXCEPTIONS");
		label10.addStyleName(ValoTheme.LABEL_H3);
		final Label label11 = new Label("Pushes ODA errors up to application-level error handling. "
				+ "Otherwise ODA handles the error internally, logging out and avoiding application-level errors as far as possible");

		final Label label12 = new Label("AUTOMIMENONE");
		label12.addStyleName(ValoTheme.LABEL_H3);
		final Label label13 = new Label("Does not automatic conversion of Items to MIME. By default, if 32k limit on a field is hit or "
				+ "any other exception is hit, the Item will be converted to MIME.");
		label13.setContentMode(ContentMode.HTML);

		final Label label14 = new Label("AUTOMIME23K");
		label14.addStyleName(ValoTheme.LABEL_H3);
		final Label label15 = new Label("Only does automatic conversion of Items to MIME if 32k limit is hit. By default, if 32k limit on a field is hit or "
				+ "any other exception is hit, the Item will be converted to MIME.");
