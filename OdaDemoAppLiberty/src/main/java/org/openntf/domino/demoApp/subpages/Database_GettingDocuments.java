...
public class Database_GettingDocuments extends BaseSubPage {
	...
	public void loadContent() {
		Label label1 = new Label("Getting documents from a database via the core Domino API has required certain knowledge of the API. "
				...
				+ "ODA also adds a number of additional methods, like making a Database act like a Map, allowing "
		...
		Label label2 = new Label("getDocumentByID(int, boolean), getDocumentByID(String), getDocumentByID(String, boolean)");
		label2.setStyleName(ValoTheme.LABEL_H3);
		Label label3 = new Label(
				"Gets a document by a String or int ID, allowing you to create the document regardless of whether it already exists. "
						+ "Historically, developers had to convert int IDs to String.");
		Label label4 = new Label("getDocumentByUNID(String), getDocumentByUNID(String, boolean)");
		label4.setStyleName(ValoTheme.LABEL_H3);
		Label label5 = new Label(
				"Allows the developer to optionally create the document with the passed UNID regardless of whether it already exists. "
		...
		Label label6 = new Label(
				"getDocumentByURL(String), getDocumentByURL(String, boolean, boolean, boolean, String, String, String, String, String, boolean)");
		label6.setStyleName(ValoTheme.LABEL_H3);
		Label label7 = new Label("These methods are unchanged from the lotus.domino versions");
		Label label8 = new Label("get(), getDocumentWithKey(Serializable), getDocumentWithKey(Serializable, boolean)");
		label8.setStyleName(ValoTheme.LABEL_H3);
		Label label9 = new Label(FactoryUtils.addCodeString("Database.get()") + " calls "
				+ FactoryUtils.addCodeString("getDocumentWithKey(Serializable)") + " which calls "
